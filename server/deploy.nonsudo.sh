...
if ! docker buildx inspect "$builder" > /dev/null 2>&1; then
    ...
    assert_success docker buildx create --name "$builder" --driver docker-container --bootstrap
fi
assert_success docker buildx use "$builder"

assert_success docker buildx build \
